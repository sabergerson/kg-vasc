...
C2S_ALL_CONFIGS = {
    "C2S_ALL_C": "c2s_all_c.yaml",
    "C2S_ALL_G": "c2s_all_g.yaml"
}

TRANSLATION_TABLE = Path(__file__).parent / "translation_table.yaml"

class C2SALLTransform(Transform):
    """Transform class for c2s_all single-cell gene expression data."""
    ...
    def run(self) -> None:
        for key in C2S_ALL_CONFIGS:
            config = Path(__file__).parent / C2S_ALL_CONFIGS[key]
