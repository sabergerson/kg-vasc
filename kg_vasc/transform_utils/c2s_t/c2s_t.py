...
C2S_T_CONFIGS = {
    "C2S_T_T": "c2s_t_t.yaml",
    "C2S_T_G": "c2s_t_g.yaml"
}

TRANSLATION_TABLE = Path(__file__).parent / "translation_table.yaml"

class C2STTransform(Transform):
    """Transform class for c2s_t single-cell gene expression data."""
    ...
    def run(self) -> None:
        for key in C2S_T_CONFIGS:
            config = Path(__file__).parent / C2S_T_CONFIGS[key]
